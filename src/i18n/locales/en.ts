...
const en: Translations = {
  ...
  aiPinAdd: '+ Pin',
  aiPinSeeMore: 'Show more',
  aiPinSeeLess: 'Show less',
  ...
  aiWorkingElapsed: (elapsed, tool) => tool
    ? `AI is working… ${elapsed} · last tool: ${tool}`
    : `AI is working… ${elapsed}`,
  aiWorkingStalled: 'Looks stuck. You can cancel and try again.',
  aiSendingOtherThread: 'AI is working in another chat.',
  aiActivitySummary: (searches, commands) => {
    const parts: string[] = [];
    if (searches) parts.push(searches === 1 ? 'Searched files' : `Searched files (${searches})`);
    if (commands) parts.push(commands === 1 ? 'Ran 1 command' : `Ran ${commands} commands`);
    return parts.join(' · ') || 'Worked in the background';
  },
  aiActivityDetails: 'Show details',
  aiChangeEdited: 'Edited',
  aiChangeKeep: 'Keep',
  aiChangeUndo: 'Undo',
  aiChangeKept: 'Kept',
  ...
  aiCustomInstructionsHint: 'Appended after the fixed safety rules. Example: reply in nynorsk, do not rewrite headings.',
  aiProjectInstructions: 'Use MERMARK.md / AGENTS.md in the workspace',
  aiWorkspaceWrite: 'Allow writing in the workspace (markdown)',
  aiWorkspaceWriteHint: 'Off by default. When on, Agent mode may update other .md files in the folder, not only the open note.',
  aiModeAsk: 'Ask',
  aiModeAgent: 'Agent',
  aiModePlan: 'Plan',
  ...
  reportFeedback: 'Report a problem',
  reportFeedbackTitle: 'Report a problem',
  reportFeedbackSubject: 'Short title',
  reportFeedbackWhatHappened: 'What happened?',
  reportFeedbackIncludeDiag: 'Include diagnostics (version, last AI error, log tails). Not the chat or the note.',
  reportFeedbackCopy: 'Copy report',
  reportFeedbackOpenIssue: 'Open GitHub issue',
  reportFeedbackEmail: 'Open email',
  reportFeedbackScreenshots: 'Screenshots',
  reportFeedbackAddScreenshot: 'Choose file',
  reportFeedbackPasteHint: 'Paste (Win+Shift+S), drop here, or choose a file.',
  reportFeedbackRemoveScreenshot: 'Remove screenshot',
  reportFeedbackPrivacy: 'Diagnostics never include the note or full chat. Screenshots can show the note. Review before you send.',
  reportFeedbackCopied: 'Report copied. Paste it into the GitHub issue.',
  reportFeedbackCopiedEmail: 'Report copied. Paste it into the email if the field is empty.',
  reportFeedbackCopiedWithShots: 'Report copied. Drag the screenshots from the folder that opened into the issue or email.',
  shortcutsTab: 'Shortcuts',
  shortcutsReset: 'Reset shortcuts',
  shortcutsPressKey: 'Press a key…',
  shortcutsConflict: 'That shortcut is already used',
  shortcutsUnbound: 'None',
  shortcutsCaptureHint: 'Click a row, then press the shortcut. Escape clears it. Enter saves.',
  ...
  workspaceFolderColor: 'Folder colour',
  workspaceFolderColorDefault: 'Default colour',
  folderColorLabel: (id) => ({
    red: 'Red',
    orange: 'Orange',
    yellow: 'Yellow',
